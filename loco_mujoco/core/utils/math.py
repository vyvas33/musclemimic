...
def calc_site_velocities(site_ids, data, parent_body_id, root_body_id, backend, flg_local=False, trajectory_site_indices=None):
    """
    ...
    Args:
        ...
        data: Mujoco/Mjx Data structure or TrajectoryData (supports sparse body fields).
        ...
        trajectory_site_indices: Optional trajectory site indices for trajectory data access.
                               If None, uses site_ids directly (for simulation data).
                               If provided, uses these indices for trajectory data access.

    Returns:
        array: Global 6D velocity vectors of the sites. Shape (batch_size, 6)

    """
    # Use trajectory indices if provided (for trajectory data), otherwise use model site IDs (for simulation data)
    site_indices_for_data_access = trajectory_site_indices if trajectory_site_indices is not None else site_ids

    site_xpos = data.site_xpos[site_indices_for_data_access]
    site_xmat = data.site_xmat[site_indices_for_data_access].reshape(*site_ids.shape, 3, 3)

    # Check for sparse body data (trajectory data with cvel_parent/subtree_com_root)
    has_sparse_cvel = hasattr(data, 'cvel_parent') and data.cvel_parent.size > 0
    has_sparse_subtree_com = hasattr(data, 'subtree_com_root') and data.subtree_com_root.size > 0

    if has_sparse_cvel and trajectory_site_indices is not None:
        # Sparse trajectory data: cvel_parent is (n_mimic, 6), indexed by trajectory site indices
        parent_body_cvel = data.cvel_parent[trajectory_site_indices]
    else:
        # Full body data (simulation or legacy trajectory): cvel is (n_bodies, 6)
        parent_body_cvel = data.cvel[parent_body_id]

    if has_sparse_subtree_com:
        # Sparse trajectory data: subtree_com_root is (3,), broadcast to all sites
        root_subtree_com = backend.broadcast_to(data.subtree_com_root, (len(site_ids), 3))
    else:
        # Full body data: subtree_com is (n_bodies, 3)
...
def calculate_relative_site_quantities(data, rel_site_ids, rel_body_ids, body_rootid, backend, trajectory_site_indices=None):
    """
    Calculate relative site quantities (positions, orientations, velocities) from trajectory or simulation data.
    
    Args:
        data: Trajectory data or simulation data with site_xpos, site_xmat attributes
        rel_site_ids: Model site IDs (used for current simulation data access)
        rel_body_ids: Corresponding body IDs for the sites
        body_rootid: Root body IDs for each body
        backend: numpy or jax.numpy
        trajectory_site_indices: Optional trajectory site indices for trajectory data access.
                               If None, uses rel_site_ids directly (for simulation data).
                               If provided, uses these indices for trajectory data access.
    
    Returns:
        Tuple of (site_rpos, site_rangles, site_rvel)
    """
    ...
    # Use trajectory indices if provided (for trajectory data), otherwise use model site IDs (for simulation data)
    site_indices_for_data_access = trajectory_site_indices if trajectory_site_indices is not None else rel_site_ids
    
    site_xpos_traj = site_xpos_traj[site_indices_for_data_access]
    site_xmat_traj = site_xmat_traj[site_indices_for_data_access]
