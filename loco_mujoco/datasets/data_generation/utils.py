...
class ExtendTrajData(ReplayCallback):
    ...
    def __init__(self, env, n_samples, model, body_names=None, site_names=None):
        ...
        for i in range(model.njnt):
            if model.jnt_type[i] == mujoco.mjtJoint.mjJNT_FREE:
                ...
                dim_qpos += 1
                dim_qvel += 1
        # Use len(self.s_ids) instead of model.nsite for site arrays since we only record selected sites
        n_selected_sites = len(self.s_ids) if site_names is not None else model.nsite
        self.recorder = dict(xpos=np.zeros((n_samples, model.nbody, 3)),
                             ...
                             site_xpos=np.zeros((n_samples, n_selected_sites, 3)),
                             site_xmat=np.zeros((n_samples, n_selected_sites, 9)),
                             qpos=np.zeros((n_samples, dim_qpos)),
                             qvel=np.zeros((n_samples, dim_qvel)))
        # Create trajectory model with selected sites if site_names is provided
        if site_names is not None:
            # Only include the selected sites in the trajectory model
            s_ids_array = jnp.array(self.s_ids)
            site_bodyid = jnp.array(model.site_bodyid)[s_ids_array]
            site_pos = jnp.array(model.site_pos)[s_ids_array]
            site_quat = jnp.array(model.site_quat)[s_ids_array]
            nsite = len(self.s_ids)
        else:
            # Include all sites
    ...
    def __call__(self, env, model, data, traj_sample, carry):
        ...
        # Handle array overflow by expanding when needed
        if self.current_length >= len(self.recorder["xpos"]):
            self._expand_arrays(model)
        ...
        self.recorder["subtree_com"][self.current_length] = (
            data.subtree_com[self.b_ids])
        
        # Validate site indices to prevent out-of-bounds access
        # This is especially important for MyoBimanualArm which has specific site filtering
        valid_site_ids = [sid for sid in self.s_ids if sid < data.site_xpos.shape[0]]
        if len(valid_site_ids) != len(self.s_ids):
            print(f"Warning: Some site IDs are out of bounds. Expected {len(self.s_ids)}, valid: {len(valid_site_ids)}")
            print(f"Site IDs: {self.s_ids}, data.site_xpos.shape: {data.site_xpos.shape}")
            # Update the site IDs to only include valid ones
            self.s_ids = valid_site_ids
            
        self.recorder["site_xpos"][self.current_length] = (
            data.site_xpos[self.s_ids])
        self.recorder["site_xmat"][self.current_length] = (
            data.site_xmat[self.s_ids])
    ...
    def _expand_arrays(self, model):
        """Expand all recorder arrays when capacity is exceeded."""
        n_expand = len(self.recorder["xpos"])
        for key, array in self.recorder.items():
            if key in ["xpos", "xquat", "cvel", "subtree_com"]:
                # Body-related arrays
                shape = (n_expand, model.nbody, array.shape[2])
                expanded = np.zeros(shape)
                self.recorder[key] = np.concatenate([array, expanded], axis=0)
            elif key in ["site_xpos", "site_xmat"]:
                # Site-related arrays
                n_sites = array.shape[1]
                shape = (n_expand, n_sites, array.shape[2])
                expanded = np.zeros(shape)
                self.recorder[key] = np.concatenate([array, expanded], axis=0)
            elif key in ["qpos", "qvel"]:
                # Joint arrays
                shape = (n_expand, array.shape[1])
                expanded = np.zeros(shape)
                self.recorder[key] = np.concatenate([array, expanded], axis=0)

    def extend_trajectory_data(self, traj_data: TrajectoryData,
                              traj_info: TrajectoryInfo):
        # Only use the actual recorded length, not the full array size
        converted_data = {}
        for key, value in self.recorder.items():
            # Slice to actual recorded length
            converted_data[key] = jnp.array(value[:self.current_length])
        return (traj_data.replace(**converted_data),
                replace(traj_info,
                       body_names=(self.b_names if len(self.b_names) > 0
                                   else None),
                       site_names=(self.s_names if len(self.s_names) > 0
                                   else None),
    ...
    def get_site_names_and_ids(model, keys=None):
        ...
        for i in range(model.nsite):
            ...
            if keys is not None:
                # Only include sites that are in the keys filter
                if s_name in keys:
                    keys.remove(s_name)
                    site_names.append(s_name)
                    ids.append(i)
            else:
                # If no filter provided, include all sites
                site_names.append(s_name)
                ids.append(i)
        assert keys is None or len(keys) == 0, f"Could not find the following site names: {keys}"
        return site_names, ids
